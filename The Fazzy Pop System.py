...
def openNewWindow():
    ...
    # Toplevel widget
    newWindow.title("Fazzy Pop's System")
    ...
    Label(newWindow,
          text="Hah just messing with you, press the exit button on both tabs to get started").pack()


label = Label(master,
              text="Welcome to Fazyy Pop Freezeria")
...
btn = Button(master,
             text="Click to open the system's menu",
...
root.title("The Fazzy Pop System")
root.configure(background='#00FFFF')

Tops = Frame(root, bg='orange', bd=20, pady=5, relief=RIDGE)
Tops.pack(side=TOP)

lblTitle = Label(Tops, font=('arial', 60, 'bold'), text='The Fazzy Pop System', bd=21, bg='black',
...
def CostofItem():
    ...
    PriceofDrinks = (Item1 * 65) + (Item2 * 75) + (Item3 * 99) + (Item4 * 90) + (Item5 * 150) + (Item6 * 75) + (
                Item7 * 75) + (Item8 * 89)

    PriceofFood = (Item9 * 300) + (Item10 * 175) + (Item11 * 255) + (Item12 * 480) + (Item13 * 240) + (Item14 * 300) + (
...
African_peri_peri_veg = Checkbutton(Food_F, text="African Peri Peri Veg", variable=var10, onvalue=1, offvalue=0,
                                    font=('arial', 16, 'bold'), bg='orange', command=chk_African_peri_peri_veg).grid(
    row=1, sticky=W)
...
Veg_Extravaganza = Checkbutton(Food_F, text="Veg Extravaganza ", variable=var13, onvalue=1, offvalue=0,
                               font=('arial', 16, 'bold'), bg='orange', command=chk_Veg_Extravaganza).grid(row=4,
                                                                                                           sticky=W)
...
Indi_Tandoori_paneer = Checkbutton(Food_F, text="Indi Tandoori paneer ", variable=var15, onvalue=1, offvalue=0,
                                   font=('arial', 16, 'bold'), bg='orange', command=chk_Indi_Tandoori_paneer).grid(
    row=6, sticky=W)
Mexican_Green_Wave = Checkbutton(Food_F, text="Mexican Green Wave ", variable=var16, onvalue=1, offvalue=0,
                                 font=('arial', 16, 'bold'), bg='orange', command=chk_Mexican_Green_Wave).grid(row=7,
                                                                                                               sticky=W)
